    ...
    <div class="d-flex justify-content-center align-items-center contenedorCardInicio" style="min-height: 100vh;">
        <div class="card text-white bg-secondary mb-3 cardInicio">
            <div class="card-header d-flex justify-content-center align-items-center">
                <p class="tituloCard">Actualizar Usuarios</p>
            </div>
            <div class="card-body">
                ...
                <form id="userForm" class="d-flex flex-column align-items-center" action="{{ route('user.update',$user->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3 w-100">
                        <select class="form-select" aria-label="Default select example" id="tipoClienteSelect"
                            ...
                            @foreach ($tiposCliente as $tipoCliente)
                                <option value="{{ $tipoCliente->id }}"
                                    {{ $user->type_id == $tipoCliente->id  ? 'selected' : '' }}>
                        ...
                        <input type="text" class="form-control no-spinner " id="ccNit" maxlength="10" name="ccNit"
                            value="{{ old('ccNit',$user->user_cc_nit) }}" required placeholder="XXXXXXXXX" disabled>
                        @error('ccNit')
                        ...
                        <input type="text" class="form-control no-spinner " id="nombre" maxlength="45" name="nombre"
                            value="{{ old('nombre', $user->user_name) }}" required disabled>
                        @error('nombre')
                        ...
                        <input type="text" class="form-control no-spinner " id="razonSocial" maxlength="45"
                            value="{{ old('razonSocial',$user->user_business_name) }}" name="razonSocial" required disabled>
                        @error('razonSocial')
                            <p>{{ $errors->first('razonSocial') }}</p>
                        @enderror
                    </div>

                    <button type="submit" class="btn btn-warning btnCard" id="registrar" disabled>Actualizar</button>
